...
把 LensRhyme 的五个创作模块作为独立 Codex skills 使用。使用你自己的 [API management Token Key](https://lensrhyme.com/api-management)，生产站点入口：[团队页](https://lensrhyme.com/team)。无需克隆 LensRhyme 应用、部署后端或配置模型供应商密钥。
...
| `lensrhyme-studio-audio` | 独立配音、旁白、Seed Audio 参考生成 | ASR、音乐创作、视频原生声音及项目绑定音频 |
...
python3 /path/to/skill-installer/scripts/install-skill-from-github.py \
  ...
  --path skills/lensrhyme-studio-image skills/lensrhyme-studio-video skills/lensrhyme-studio-audio skills/lensrhyme-canvas skills/lensrhyme-workbench
```
...
所有五个 skill 均使用 `Authorization: Bearer` 和可选 `X-Workspace-ID`。支持原有 IP 白名单、权限及模型访问控制。此密钥不是 OpenAI、火山引擎或其他供应商 API Key。本仓库不包含测试账号密码或任何有效凭证。

## Studio 默认模型

| 类型 | 默认模型 | API model ID |
| --- | --- | --- |
| 音频 | seed-audio-1.0 | `seed-audio-1.0` |
| 图片 | doubao-seedream-5.0-pro | `doubao-seedream-5-0-pro-260628` |
| 视频 | doubao-seedance-2.0-fast | `doubao-seedance-2-0-fast-260128` |

默认模型写入调用脚本，使用 `studio image/video/audio --json-file payload.json` 时自动填充。`request POST /tasks/` 且 `entrypoint: studio` 也会补全；用户显式指定其他模型时保留该选择。默认值仅作用于这些技能的 Studio 任务，不修改生产账号设置、Canvas 或 Workbench 的模型配置。同步 `/image/...` 或 `/video/...` 接口需显式传入默认模型 ID。
...
图片/视频/音频 Studio 任务 body 示例、Canvas 关联方式、Workbench 结构与导出流程见各自 `references/api.md`。每个 skill 都有生产 OpenAPI 的模块子集，`schema` 命令无需凭证即可查阅。示例中的模型参数需与账户实时模型能力核对。
...
客户端源码位于 `scripts/lensrhyme_api.py`，发布副本放进每个 skill 以支持独立安装。更新客户端时同步五份副本；测试会验证一致性。接口快照日期为 2026-09-21，更新契约时从部署的 `/api/v1/openapi.json` 提取模块接口及其引用 schema，避免发布用户数据或内部配置。
